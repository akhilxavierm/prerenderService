...
function preCacheCall(allUrls) {
  for (var i = 0; i < 5; i++) {
    ...
    request({
      ...
      if (!error && response.statusCode == 200) {
        urlCount++;
        console.log("url count success--" + (urlCount - 1));
        if (urlCount < allUrls.length) {
          singlePreCacheCall(allUrls);
        }

      } else {
        console.log("url count error--" + urlCount);
        if (urlCount < allUrls.length) {
          singlePreCacheCall(allUrls);
        }
      }
...
function singlePreCacheCall(allUrls) {
  console.log("url count in singleprecache Call--" + urlCount);
  clearRedis(allUrls[urlCount]);
  request({
    headers: {
      'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.8; rv:24.0) Gecko/20100101 Firefox/24.0'
